...
let modulePromise: Promise<typeof import("./wasm/idml_wasm")> | null = null;
...
async function loadModule() {
  if (!modulePromise) {
    modulePromise = (async () => {
      ...
      // `new URL(...)` natively.
      const mod = await import("./wasm/idml_wasm");
 ...
 * Mirrors `idml_wasm::render_pages(idml, font, dpi)` and copies each
