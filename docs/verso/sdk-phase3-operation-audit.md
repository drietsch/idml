...
**Critical-path single risk (matching plan §11):** the character/paragraph addressing model. Today's `NodeId::TextFrame(_)` doesn't address ranges. Either Phase 3 makes the simplifying assumption "apply Character properties to the whole frame as an override on the first range" (cheap, lossy on multi-style frames) or it extends `NodeId` / `PropertyPath` to carry a range. The plan's binding ceiling (§11.5: "literals + selectionProperty refs + unit coerce only") does not specify which — that's the design decision Phase 3 must make before the four named character paths can ship.
