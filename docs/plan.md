...
| Overprint simulation | ✅ parser + display list + Stage 3 CPU darken + Stage 4A per-channel CMYK composite when source paint is CMYK + Stage 4B full CMYK plane pipeline on the CPU rasterizer; Stage 4C (spot ink planes) deferred — see Tier 3 #14 |
...
14. **Overprint simulation** — Stages 1–3 ✅, Stage 4 split into **4A ✅ / 4B ✅ (CPU rasterizer; Vello backend retained on Stage 4A) / 4C deferred**. *Parser*: `OverprintFill` / `OverprintStroke` lift off `<Rectangle>` / `<Oval>` / `<Polygon>` / `<TextFrame>` / `<GraphicLine>` (GraphicLine carries stroke only) and off `<CharacterStyleRange>` / `<ParagraphStyleRange>` / `<CharacterStyle>` / `<ParagraphStyle>`; the cascade merges them via `ResolvedCharacter::merge_below` / `ResolvedParagraph::merge_below`. *Display list*: distinct `FillPathOverprint` / `StrokePathOverprint` variants — chosen over a flag on `FillPath` so the knockout fast path stays a single match arm and existing construction sites need no churn. `ResolvedFrame` carries `overprint_fill` / `overprint_stroke` flags; `module::geometry::rewrite_tail_for_overprint` upgrades the emitter's tail of `FillPath` / `StrokePath` commands to the `*Overprint` variant after fill/stroke modules emit. *Stage 3 (CPU RGB darken)*: each `*Overprint` command rendered into a scratch pixmap sized to the path's pixel bbox (+ stroke pad), then `draw_pixmap`'d onto the target with `tiny_skia::BlendMode::Darken` — per-channel `min(top, bottom)`. Retained as the fallback path for non-CMYK source paints (RGB swatches, gradients) and for paint shapes whose provenance isn't CMYK-traceable. *Stage 4A (CMYK-aware paints + per-channel overprint composite) ✅*: new `Paint::Cmyk { c, m, y, k, rgb }` variant carries source ink channels (0..=1) end to end alongside the cached ICC-resolved display RGB. `color_id_to_paint` returns `Paint::Cmyk` whenever the swatch is `Space="CMYK"` (process or spot-with-CMYK-alternate) and an `IccTransform` is available; the cached `rgb` field keeps ordinary (non-overprint) draws bit-identical to the prior `Paint::Solid` path (no fidelity regression — see effects / text-advanced / gradients gates). On overprint, `compose_cmyk_overprint_at` walks the path's scratch pixmap pixel-by-pixel: decodes each destination pixel back to CMYK via the inverse naive Adobe map (`rgb_to_naive_cmyk_8bit`), takes per-channel `max(top, bottom)` against the source CMYK channels (scaled by source coverage for anti-alias correctness), forward-converts to RGB (`naive_cmyk_to_rgb_8bit`), and writes the new pixel back. The inverse round-trips exactly through the forward map at 8-bit precision for any pixel that *was itself produced by a CMYK swatch through the naive forward path* — which is the production case the brief calls out (two CMYK swatches meeting each other). For non-CMYK destinations the per-channel max still produces a sensible darken in inferred-CMYK space; for non-CMYK sources we fall through to the Stage 3 `tiny_skia::Darken` path. *Vello (GPU)*: still falls back to a knockout fill on overprint commands (the Stage 4A per-pixel walk is CPU-rasterizer-specific; replicating it on Vello needs a custom shader). *Stage 4B (CPU plane pipeline) ✅*: parallel C/M/Y/K accumulator buffers + a coverage mask owned by `rasterize`. Lazily allocated on the first `Paint::Cmyk` draw to the page (no memory cost when no CMYK draws happen). Every CMYK-paint draw splats its path coverage into the planes via `splat_scratch_into_planes` (per-channel `max(existing, ink·coverage)`) **and** paints the cached `Paint::Cmyk { rgb }` into the RGB framebuffer normally — so non-overprint draws stay byte-identical to Stage 4A's output. `FillPathOverprint` / `StrokePathOverprint` with a CMYK paint targeting the page route through `compose_cmyk_overprint_via_planes`, which reads destination CMYK directly from the planes (no inverse-RGB recovery), takes per-channel `max(top, bottom)` with coverage-weighted source ink, writes the new CMYK back to the planes, and updates the RGB framebuffer inline through `naive_cmyk_to_rgb_8bit`. This means a chain of overprints accumulates ink correctly — the second overprint sees the first one's plane state rather than re-inferring it from the framebuffer pixel each time, which was Stage 4A's intrinsic ceiling. Group-buffer renders (inside `BeginBlendGroup` / `PushLayer`) keep the Stage 4A inverse-RGB path; plane state is page-level only, the cross-boundary plane plumbing for nested groups is more rework than the visible-fidelity payoff justifies today. Vello backend stays on Stage 4A. The brief's "ICC-convert the planes at flush time" pass is intentionally NOT wired in: every plane write already updates the framebuffer in step (ICC-resolved for normal draws, naive 8-bit for overprint), so an end-of-render flush would only diverge from the in-sync framebuffer at the math-path boundary (mean ΔE ~0.2 over the geometry corpus when run unconditionally). The `flush_cmyk_planes_into_rgb` helper is kept (with `#[allow(dead_code)]`) as the primer for Stage 4C spot-ink plumbing. Coverage: `idml-gpu::cpu::tests::cmyk_plane_pipeline_overprint_after_normal_cmyk_layer_yields_per_channel_max` (overprint reading prior-normal-CMYK plane state), `cmyk_plane_pipeline_chained_overprints_accumulate_ink_per_channel` (two overprints, plane→plane composition), `cmyk_plane_flush_leaves_non_cmyk_pixels_unchanged` (no plane allocations / no flush touches for pure-RGB renders). All 9 generated-corpus fixtures stay within their tightened thresholds (`./corpus/generated/diff.sh`), and the real-world `manual-sample.idml` numbers come out bit-identical to Stage 4A (its CMYK ranges don't exercise overprint). *Stage 4C (deferred)*: spot inks rendered as named-ink planes, not via the CMYK alternate fallback. Today `ColorEntry::effective_cmyk` collapses every spot to its CMYK alternate before the renderer sees it; Stage 4C would preserve the spot identity as a separate plane (e.g. `Plane::Spot(ink_name)`) so an InDesign-PDF-equivalent separations export becomes possible. *Gaps documented*: (a) the "K=100 is auto-overprint regardless of attribute" Adobe default isn't honoured — we honour only what the IDML attribute says; (b) text runs don't yet route overprint through the glyph emitter (run-level cascade lands in `ResolvedRunAttrs` but the glyph paint module doesn't consume it yet — frame-level overprint is wired). Coverage: `idml_parse::spread::tests::overprint_attributes_round_trip_through_every_shape`, `idml_parse::story::tests::overprint_round_trips_on_paragraph_and_run`, `idml-renderer/tests/overprint.rs::overprint_fill_darkens_top_color_against_bottom_color` (cyan-on-magenta darken assertion + black-on-yellow knockout check), `idml-gpu::cpu::tests::cmyk_overprint_cyan_on_magenta_produces_per_channel_max` (recovers CMYK from rendered pixel and pins C ≥ 250 ∧ M ≥ 250 ∧ Y ≤ 5 ∧ K ≤ 5 — the per-channel-max invariant that Stage 3 RGB darken merely *coincidentally* satisfied for this fixture), `idml-gpu::cpu::tests::cmyk_paint_non_overprint_renders_identically_to_solid_path` (regression-guard: pure-CMYK direct draw still renders ~(0,255,255) without overprint).
...
- `idml-gpu`: 22 unit (CPU default; CMYK Stage 4A added `cmyk_overprint_cyan_on_magenta_produces_per_channel_max` + `cmyk_paint_non_overprint_renders_identically_to_solid_path`. +vello-backend adds 7 — 3 encoder smoke tests + 3 push-layer/blur-specific tests + the GPU-availability-guarded blur halo assertion)
...
- **Total: 375 across the workspace** (CPU default features). Accumulated additions: justification enum (+2 parse), numbering-polish (+3 parse, +7 renderer unit, +3 `text_glyph_level`), bullet character style (+1 `text_glyph_level`), decimal-tab leaders (+1 parse, +1 `text_glyph_level`), drop-shadow-blur (+3 `idml-gpu`), tables-parser (+2 parse), tables-flow-polish (+1 parse, +2 `text_glyph_level`), CJK parser surface (+7 in `story::tests`, +3 in `styles::tests`), CJK Stage 2 kinsoku (+7 in `compose::tests`), CMYK overprint Stage 4A (+2 `idml-gpu` — per-channel-max contract + non-overprint no-regression guard).
