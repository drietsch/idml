...
| 2 | Test font + glyph-level integration tests ✅ | Done — fonts landed in `corpus/fonts/`, glyph-level coverage in `text_glyph_level.rs` (per-run fonts, threading, underline + strikethrough, vertical justify, bullets, numbered list). |
