...
| TOC — renderer integration | ❌ deferred. Real-world IDMLs serialise the resolved TOC story as plain paragraphs at export time (the current renderer renders those correctly as ordinary text). Replacing an *unresolved* TOC story's content with `resolve_toc(...)`'s output needs a hook in the body-story emission loop (skip the original story, synthesise a paragraph per entry under the entry's `FormatStyle`, expand `^t` in `Separator`, append `format_number(page_number)`). Our generated corpus carries no unresolved-TOC fixture, so the renderer path stays unwired pending one — per CLAUDE.md's "don't ship half-finished renderer plumbing" convention. The resolver is callable today; the integration is mechanical when a fixture lands. |
...
- `idml-parse`: 74 unit + 3 integration (`roundtrip`) — Tables coverage extended (+3) with `parses_table_with_header_body_footer_and_corner_cells` (3×3 grid, all four corners, header/body/footer counts), `parses_multi_paragraph_cell_content` (multiple `<ParagraphStyleRange>` per cell), and `parses_table_repeating_header_footer_and_row_max_min_height` (`RepeatingHeader` / `RepeatingFooter` booleans + `MaximumHeight` on rows); plus `tab_stop_leader_preserves_multichar_and_whitespace`. TOC parser landed (+2) with `parses_toc_style_with_entries` (Title / TitleStyle round-trip + per-entry Level / IncludeStyle / FormatStyle / PageNumber / Separator) and `parses_self_closing_empty_toc_style` (the default `<TOCStyle ... />` real-world IDMLs always ship).
